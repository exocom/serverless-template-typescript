...
import {ApiGatewayHandler, LambdaUtil} from './utils/api-gateway-util';
...
export const getUsers: ApiGatewayHandler = async (event, context) => {
  ...
  return lambdaUtil.apiResponse({body: {data: users}});
};
...
export const postUsers: ApiGatewayHandler = async (event, context) => {
  try {
    ...
    if (errors && errors.length) return lambdaUtil.apiResponse({statusCode: 400, body: {errors}});
    ...
    return lambdaUtil.apiResponse({statusCode: 201, body: {data: userDocument}});
    ...
    return lambdaUtil.apiResponse({statusCode: 500, body: {errors}});
  }
};
...
export const getUser: ApiGatewayHandler = async (event, context) => {
  ...
  if (!user) {
    ...
    return lambdaUtil.apiResponse({statusCode: 404, body: {errors}});
  }

  return lambdaUtil.apiResponse({body: {data: user}});
};
